...
调用者的 WebDAV/备份根。它包含生成时的普通逻辑 JSON 以及：
...
2026-07-22 工作台复审后，“保存完整本地书备份”保留为明确命名的直接动作；普通保存也是
确认后的直接动作。备份列表、上传、下载和恢复统一归唯一 WebDAV 文件管理器，portable 行标为
“含本地书原文件”，恢复确认说明将恢复本地书。删除重复管理器不改变 portable manifest、
archive 内容、API 路径或恢复分流。
...
4. 所有包控制的失败（ZIP/manifest/hash/解析/identity 冲突/限额）必须在第一条逻辑恢复写入前
   ...
   时删除本次 archive；持久写入成功后才发送 bookshelf/progress/bookmark sync 事件。普通
   reader-dev/Legado/OpenReader 逻辑 JSON 的新 planner/SQLite 事务由
   [`backup-restore-fixed-baseline-p2-contract.md`](backup-restore-fixed-baseline-p2-contract.md)
   管理；这不把 portable 的数据库与 archive 文件系统声明成跨介质全局原子提交。极端的磁盘/
   数据库运行时故障仍按本合同的 staging/补偿边界处理。
