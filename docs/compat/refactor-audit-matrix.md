...
| 书架、BookManage、BookGroup、BookInfo | `BookShelf.vue`、`BookManage.vue`、`BookGroup.vue`、`BookInfo.vue` | `Home.vue`、`OverlayBookManagement.vue`、`OverlayBookGroups.vue`、`BookInfoDialog.vue`/`BookInfoPanel.vue` | **刷新阅读进度 P1 已随 `ed4ee27` 发布**：完整网络快照现按上游整批替换非 pending 进度，服务器空值清除对应客户端记录；只有真实 `pendingSync` 暂存并 CAS 重试。前端 520/520、Go/build、断开 WebSocket + 2099 年旧缓存的三视口浏览器及新旧卷/备份门禁通过。BookGroup 与独立 `lastCheckTime` 合同继续有效。 | [`bookshelf-refresh-progress-p1-contract.md`](bookshelf-refresh-progress-p1-contract.md)；等待用户在真实多设备书架复验，其它模块继续按矩阵审查。 |
