...
| 本地导入、书仓、WebDAV | `BookController.kt`、`LocalBook.kt`、`TextFile.kt`、`WebDAV.vue` | `OverlayBookImport.vue`、`LocalStore.vue`、`WebDAVBrowser.vue`、`OverlayStorageImport.vue`、`useStorageImportWorkflow.js`、`backend/services/localbook/*`、`engine/*_parser.go` | **P1-E1、P1-E2、P1-E3 已完成**：失败预览 token 重试和唯一导入 controller 已验证；P1-E3 已按专门合同重建当前目录表格、上传多选、格式入口和移动端同表格语义，额外目录/重命名/递归/下载 UI 已从工作台移除。完整真实格式和旧卷复验仍未开始。 | `docs/compat/workspace-storage-import-p1e-contract.md`、`docs/compat/workspace-storage-import-p1e3-contract.md` → P1-E4 真实文件夹/卷/Docker smoke。 |
