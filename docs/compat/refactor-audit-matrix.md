...
| Go REST、鉴权与错误语义 | Kotlin `*Controller.kt`、ReturnData、`YueduApi.kt` `/assets/*` | `backend/api/*.go`、middleware、前端 `api/*.js`、public capability routes | **按动作逐项复审；已关闭模块不从旧日志重开**。Book/Category 单项 patch 列所有权已发布。当前直接差集收敛到 `POST /api/books/batch` 的三个 category action：关系读取绕过 tx/context，且以 `tx.Save` 提交完整 Book 快照。 | [`batch-book-category-write-lifecycle-fixed-baseline-second-audit-p2-contract.md`](batch-book-category-write-lifecycle-fixed-baseline-second-audit-p2-contract.md) 已固定 request-context 同一 tx 关系读写、guarded `category_id`、取消回滚、删除不复活与权威 event；状态 `inventory-complete / tests-and-implementation-pending`。已签收 BookManage wire/UI、单本 category 与 delete/cache 合同不重开。 |
...
## 当前整体进度快照（2026-08-31，Batch Book Category lifecycle inventory）
...
并发提交亦已关闭；Category patch full-row `Save` 差集现亦已测试先行实施、发布。剩余约 1% 当前包含已确认的 batch category contextless relation read/full-row `Save` must-fix、后续逐路由
