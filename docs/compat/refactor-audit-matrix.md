...
| Reader：工具层、面板、正文、翻页 | `Reader.vue`、`Content.vue`、`ReadSettings.vue`、`PopCatalog.vue`、`BookShelf.vue`、`BookSource.vue` | `views/Reader.vue`、`components/reader/*`、`composables/useReader*`、`stores/reader.js` | 工具层状态机、格式运行时、设置、夜间面、远程正文和换源写入生命周期均已专项实施。**第四次章节真机反馈定位到 `0a8a0ef` 对 detached source snapshot 的抓取前/提交后合同冲突；`8bebcbf` 已允许既有书籍继续通过其 detached source 阅读，同时保持关联缺失、换源、删书、目录与语义变化的安全 409**。`c7fbf73` 的 30 秒正文预算、请求接管，`0ecc4d9` 的同章 gate/相邻章并行和 staged CAS 均保留。桌面翻页维持 document-root、30/40/30 点击区和原生滚轮/触控板；固定章节运行栏保留。 | `8bebcbf` 已通过 frontend 757/757、Go full/vet/race、build/Compose、真实 detached-source API 及三视口 Chromium；可信 Actions run `34963585121` 已发布 `8bebcbf`/`latest` OCI index `sha256:5d097551c7d5c37bc54b69030ef07146d7b24888583ba2abc3903b7eff8d6a03`。状态 `implemented / regression-validated / Docker-published / awaiting-device-verification`。 |
...
| 测试、构建、Docker、卷升级 | 上游功能契约；OpenReader Docker/data 约束 | `frontend/tests`、`scripts/smoke`、`backend/**/*_test.go`、Dockerfile、release workflow | `8bebcbf` 本地通过 frontend 757/757、Go focused/race/full/vet、build、Compose 与真实 detached-source 三视口。可信 Actions run `34963585121` 又通过 backend/frontend/build/Compose、native、fresh/portable、historical volume 与 published-platform 门。 | Docker commit 为 `8bebcbf`；已发布 `8bebcbf`/`latest` OCI index `sha256:5d097551c7d5c37bc54b69030ef07146d7b24888583ba2abc3903b7eff8d6a03`。amd64 manifest `sha256:d805484871070bbb6215e48015da14cc58b78a2bcc33bc1c584516cac636f371`，arm64 manifest `sha256:dccb59866f3b9a611fbc0d81286f6b4d1ddbcd8774f20f91c67e83cda6478cb5`；文档记录提交将晚于镜像 commit，用户生产环境运行提交未知。 |

## 当前整体进度快照（2026-09-15，Reader detached source 回归已实施发布）
...
- **P0 Reader 主链已覆盖**：工具层/面板状态机、正文排版、移动点击与连续滚动、设置、书签、正文
  ...
  transport 与调用方订阅。第四次反馈最终定位到 `0a8a0ef` 的 detached source 提交条件与抓取条件
  冲突；`7e83024`/`015d255`/`8bebcbf` 已按合同、红测、实现顺序统一为 existing association，并以
  真实 Go/SQLite/API/Chromium 三视口证明 detached source 只抓取一次、正文 200、缓存发布且不被重新
  激活。关联缺失及真实生命周期变化仍 fail closed；可信 Actions 已完成双架构发布，用户真机仍待复验。
