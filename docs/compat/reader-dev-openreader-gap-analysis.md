...
| Online source parsing | Upstream reader3-compatible source semantics live across `AnalyzeRule` plus `BookList/BookInfo/BookChapterList/BookContent`. | Current Go parser executes the extracted CSS/JSONPath/XPath/regex/composite/replace/pagination subsets and bounded request-scoped `@put`/`@get`; JS/templates remain explicit errors. | Request variables flow only within one safe operation and isolate search items/multi-branches. Upstream cross-request book/chapter variables still need a separate migration and multi-user contract. `{{...}}` JavaScript remains an explicit security-gated difference, never a silent empty result. | `partial` for P2 parser | Golden parser fixtures plus request/user isolation tests; source-debug/error-redaction tests; browser source flow. |
