...
正文图片离线缓存的下一切片已完成实现与自动验证，见
[`book-cache-images-p2-contract.md`](book-cache-images-p2-contract.md)，其 Docker 卷/发布证据单独记录。此前
...
- 本提交没有 SQLite schema、挂载根、备份字段或缓存路径格式迁移；其 `06ac89b` 发布仍只声明
  “整本正文缓存”对齐。后续图片切片已按独立合同完成实现与自动验证，等待自己的可追踪 Docker
  卷/备份门禁，不能倒推为 `06ac89b` 已具备图片离线能力。
