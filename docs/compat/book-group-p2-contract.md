...
- 真实浏览器：`scripts/smoke/book-group-real-api-contract.mjs` 使用真实 Go/SQLite 在 1440×900、
  390×844、360×800、1024×1366、1366×1024 通过；覆盖四内置管理行、非空书架标签、内置音频改名、
  显隐、多客户端同步、混合拖拽与刷新后的持久化。音频筛选语义继续由显式 `type=1` 单元合同锁定；
  普通 `POST /api/books` 夹具不再伪造受保护的 `type/sourceId` 字段。
