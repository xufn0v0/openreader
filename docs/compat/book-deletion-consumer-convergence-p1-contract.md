...
| Shelf/cache consumer | Reloaded upstream lists no longer expose the deleted item. | Direct and sync paths remove shelf memory/cache rows and the captured account's scoped browser chapter cache. They do not clear `reader.progressByBook` or scoped/legacy local progress. | `must-fix`: every confirmed deleted id clears shelf entries, scoped chapter cache and local reading progress before a future sort/reopen can consume them. Unowned upstream-era browser cache remains untouched and unreadable by authenticated accounts. |
...
- The deletion transaction freezes the authenticated scope before asynchronous
  ...
  cleanup is pending; cache key formats remain unchanged. The 2026-07-27
  cache-scope hardening additionally stops treating an unscoped upstream-era
  chapter key as owned by every authenticated user.
