...
> 上游源码快照：`/private/tmp/hectorqin-reader/web/src`  
...
- 桌面保留侧边导航；移动端使用底部主导航 + 页面内抽屉。
- 书架页移动端第一屏只保留：最近阅读、搜索、导入、书架列表。
- 管理能力全部进抽屉/全屏弹层。
- 表格页移动端不能硬塞列，应改成卡片列表或可横向滚动。
