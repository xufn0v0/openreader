...
test('every delayed authenticated store path owns a current operation before committing', () => {
  ...
  assert.match(bookshelf, /bookGroupOperations\.begin\('book-groups'\)/)
  assert.match(bookshelf, /scopedShelfCacheKey\(BOOK_GROUP_CACHE_KEY, scope\)/)
...
test('websocket callbacks are bound to their candidate generation and authenticated identity', () => {
  ...
  assert.match(sync, /message\.type === 'book_groups_update'/)
  assert.match(sync, /bookshelf\.replaceBookGroups\(message\.payload\)/)
})
