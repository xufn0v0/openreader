...
test('preserves a preview import token through LocalStore/WebDAV confirmation', () => {
  ...
  assert.match(overlay, /importFromLocalStore\(\[backendItem\], categoryIds, options\)/, 'the root controller must submit each LocalStore item separately')
  assert.match(overlay, /importFromWebDAV\(\[backendItem\], categoryIds, options\)/, 'the root controller must submit each WebDAV item separately')
...
test('keeps direct TXT empty catalogues visible as a recoverable successful preview', () => {
  assert.match(overlay, /currentRow\.chapterCount\s*===\s*0/, 'a successful empty catalogue must be distinct from a parser failure')
  assert.match(overlay, /未匹配到目录；可调整规则后刷新目录，或保留空目录继续导入。/, 'the shared dialog must explain that users can adjust the rule or retain the empty catalogue')
})
