...
test('aligns RSS source-open and close transitions with the upstream modal chain', () => {
  ...
  assert.match(manager, /async function selectSource\(sourceId\)\s*\{[\s\S]*?resetSourceArticleState\([^)]*\)[\s\S]*?articleListDialogVisible\.value = true[\s\S]*?await refreshSelectedSource\(operation\)/, 'source selection must reset stale article state, open the list dialog, and refresh the selected source like upstream')
  assert.match(manager, /async function handleSortChange\(\)\s*\{[\s\S]*?resetSourceArticleState\(\)[\s\S]*?await loadArticles\(operation\)[\s\S]*?await refreshSelectedSource\(operation\)/, 'sort changes must reset the old page, show scoped cache, then refresh page one')
  assert.match(manager, /async function refreshSource\(source, parentOperation = null\)[\s\S]*?await loadArticles\(operation\)/, 'a selected-source refresh must render the refreshed/cached rows')
  ...
  assert.match(manager, /async function loadArticles\(parentOperation = null\)[\s\S]*?if \(!props\.visible \|\| !articleListDialogVisible\.value\) return[\s\S]*?operations\.canCommit\(operation\)[\s\S]*?articleListRequestGate\.isCurrent\([\s\S]*?articles\.value = result\.items/, 'an article request finishing after child/root close, account switch, or a newer query must not restore stale rows')
})
