...
const discoverPath = resolve(__dirname, '../src/views/Discover.vue')
const explorePopoverPath = resolve(__dirname, '../src/components/workspace/ExploreWorkspacePopover.vue')
...
test('makes the root route the canonical shelf, search, and explore workspace', () => {
  ...
  assert.match(home, /<SearchWorkspace\s+v-if="workspace\.isSearchResult"/)
  assert.match(home, /<DiscoverWorkspace\s+v-else-if="workspace\.isExploreResult"/)
  assert.doesNotMatch(home, /<SearchWorkspace[^>]*\bembedded/)
  assert.doesNotMatch(home, /<DiscoverWorkspace[^>]*\bembedded/)
  assert.match(home, /workspace\.beginSearch\(/)
  assert.match(home, /workspace\.requestExplore\(/)
  assert.match(home, /workspace\.backToShelf\(\)/)
})

test('opens the upstream Explore chooser before changing the root result body', () => {
  const layout = readFileSync(layoutPath, 'utf8')
  const discover = readFileSync(discoverPath, 'utf8')
  const popover = readFileSync(explorePopoverPath, 'utf8')
  ...
  assert.match(popover, /class="explore-workspace-popover"/)
  assert.match(popover, /listExploreSources/)
  assert.match(popover, /workspace\.showExploreResults\(/)
  assert.doesNotMatch(popover, /workspace\.beginExplore\(/, 'opening the chooser must not preemptively replace the shelf')
  assert.match(layout, /if \(isMobileShell\.value && item\.closeMobile\)/)
