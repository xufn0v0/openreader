...
test('RSS list and load-more responses are committed through request ownership gates', () => {
  ...
  assert.match(manager, /async function loadArticles\(\)[\s\S]*?articleListRequestGate\.begin\([\s\S]*?articleListRequestGate\.isCurrent\(/)
  ...
  assert.match(manager, /async function handleFilterChange\(\)[\s\S]*?resetSourceArticleState\(\)[\s\S]*?await loadArticles\(\)/)
  assert.match(manager, /async function selectSource\(sourceId\)[\s\S]*?const query = articleRequestQuery\(1\)[\s\S]*?await loadArticles\(\)[\s\S]*?if \(!isArticleRequestQueryCurrent\(query\)\) return[\s\S]*?await refreshSelectedSource\(\)/)
  assert.match(manager, /async function handleSortChange\(\)[\s\S]*?const query = articleRequestQuery\(1\)[\s\S]*?if \(!isArticleRequestQueryCurrent\(query\)\) return[\s\S]*?await refreshSelectedSource\(\)/)
})
